...
class BaseSampler {
 public:
  ...
  static void GenerateMipmaps(BaseTexture<T> *tex);
...
template<typename T>
void BaseSampler<T>::GenerateMipmaps(BaseTexture<T> *tex) {
  ...
  tex->mipmaps_thread = std::make_shared<std::thread>([&]() {
    ...
    tex->mipmaps.push_back(BaseTexture<T>::CreateBuffer(tex->buffer->GetLayout()));
    Buffer<Vec4<T>> *level_buffer = tex->mipmaps.back().get();
    Buffer<Vec4<T>> *level_buffer_from = tex->buffer.get();
