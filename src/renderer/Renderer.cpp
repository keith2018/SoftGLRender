...
void Renderer::Create(int w, int h, float near, float far) {
  width_ = w;
  height_ = h;
  ...
  graphic_.DrawPoint = std::bind(&Renderer::DrawFramePointWithDepth, this, std::placeholders::_1,
...
void Renderer::DrawMeshTextured(ModelMesh &mesh, glm::mat4 &transform) {
...
void Renderer::DrawMeshWireframe(ModelMesh &mesh, glm::mat4 &transform) {
  render_ctx_.Prepare(&mesh, shader_context_.varyings_size / sizeof(float));
  ProcessVertexShader();
...
void Renderer::DrawLines(ModelLines &lines, glm::mat4 &transform) {
...
void Renderer::DrawPoints(ModelPoints &points, glm::mat4 &transform) {
...
void Renderer::ProcessVertexShader() {
  for (auto &vh : render_ctx_.vertexes) {
    VertexShaderImpl(vh);
  }
}

void Renderer::VertexShaderImpl(VertexHolder &vh) {
...
void Renderer::ProcessFaceFrustumClip() {
  if (!frustum_clip) {
    return;
  }
...
void Renderer::ProcessFaceScreenMapping() {
...
void Renderer::ProcessFaceFrontBackCull() {
  for (auto &fh : render_ctx_.faces) {
    if (fh.discard) {
      continue;
    }
...
void Renderer::ProcessFaceRasterization() {
  // early depth test
...
void Renderer::TriangularRasterization(FaceHolder &face_holder, bool early_z_pass) {
...
void Renderer::PixelQuadBarycentric(PixelQuadContext &quad, bool early_z_pass) {
...
void Renderer::BarycentricCorrect(PixelQuadContext &quad) {
  glm::aligned_vec4 *vert = quad.vert_flat;
...
void Renderer::PixelShading(glm::vec4 &screen_pos, bool front_facing, BaseFragmentShader *frag_shader) {
...
bool Renderer::DepthTestPoint(int x, int y, float depth) {
  if (!depth_test) {
    return true;
  }
...
glm::vec4 Renderer::Viewport(float x, float y, float w, float h) {
  glm::vec4 m;
  m.x = w / 2.f;
  m.y = h / 2.f;
  m.z = x + m.x;
...
void Renderer::PerspectiveDivide(glm::vec4 &pos) {
...
void Renderer::ViewportTransform(glm::vec4 &pos) const {
...
int Renderer::CountFrustumClipMask(glm::vec4 &clip_pos) {
  int mask = 0;
...
BoundingBox Renderer::VertexBoundingBox(glm::vec4 *vert, size_t w, size_t h) {
...
void Renderer::VaryingInterpolate(float *out_vary,
