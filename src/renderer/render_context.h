...
struct RenderContext {
  ...
  void Prepare(ModelMesh *mesh, size_t vary_size = 0) {
    ...
    if (varyings_aligned_size > 0) {
      varyings_pool = std::shared_ptr<float>((float *) Memory::AlignedMalloc(varyings_aligned_size * vertexes.size()),
                                             [](const float *ptr) { Memory::AlignedFree((void *) ptr); });
  ...
  VertexHolder VertexHolderInterpolate(VertexHolder *v0, VertexHolder *v1, float weight) const {
    ...
    ret.clip_mask = 0;
    if (varyings_aligned_size > 0) {
      ret.varyings_append = std::shared_ptr<float>((float *) Memory::AlignedMalloc(varyings_aligned_size),
                                                   [](const float *ptr) { Memory::AlignedFree((void *) ptr); });
...
struct PixelQuadContext {
  ...
  explicit PixelQuadContext(size_t varyings_aligned_size) {
    if (varyings_aligned_size > 0) {
      varyings_out_pool = std::shared_ptr<float>((float *) Memory::AlignedMalloc(4 * varyings_aligned_size),
                                                 [](const float *ptr) { Memory::AlignedFree((void *) ptr); });
      for (int i = 0; i < 4; i++) {
